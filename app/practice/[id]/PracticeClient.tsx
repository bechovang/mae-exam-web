...
import {
  ...
  HelpCircle,
  Target,
  Keyboard, // Imported Keyboard icon
} from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
...
export default function PracticeClient({ practiceId }: PracticeClientProps) {
  ...
  const [showShortcutsHelp, setShowShortcutsHelp] = useState(false) // State for shortcuts dialog
  ...
  const useKeyboardShortcuts = () => {
    const handleKeyPress = useCallback((event: KeyboardEvent) => {
      ...
      if (showSummaryDialog || showShortcutsHelp) {
        if (event.key === 'Escape') {
      ...
      switch (event.key.toLowerCase()) {
        ...
        case 'escape': event.preventDefault(); router.push('/select-exam'); break;
        ...
        default:
          if (event.ctrlKey && !isNaN(parseInt(event.key))) {
            ...
            if (practiceData && questionIndex >= 0 && questionIndex < practiceData.questions.length) {
        ...
        showFeedback, practiceData, currentQuestion, questionResults, showSummaryDialog, showShortcutsHelp, 
        handleAnswerSelect, goToPrevQuestion, goToNextQuestion, resetQuestion, toggleHints, goToQuestion, router
    ]);
      ...
      try {
        ...
        setPracticeData(response.ok ? await response.json() : {
          ...
          questions: Array.from({ length: 10 }, (_, i) => ({
            id: i + 1, question: `Câu hỏi ${i + 1}: Giải phương trình sau \\( x^2 - 4x + 4 = 0 \\)`, image: null,
            options: ["A. \\(x=1\\)", "B. \\(x=2\\)", "C. \\(x=3\\)", "D. \\(x=4\\)"], correctAnswer: "B",
            explanation: `Đây là giải thích chi tiết cho câu hỏi ${i + 1}. Phương trình có nghiệm kép \\(x=2\\) vì \\( (x-2)^2 = 0 \\).`,
            difficulty: i % 3 === 0 ? "easy" : i % 3 === 1 ? "medium" : "hard", topic: "Đại số",
  ...
  const getStudyTip = () => { /* ... (same as original) ... */ };
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-black p-4">
      <div className="mx-auto w-full max-w-4xl">
              ...
              {currentQuestionData.options.map((option, index) => {
                ...
                return (
                  <div key={index} className={optionClass}>
                    ...
                    {!showFeedback && <Badge variant="outline" className="text-xs ml-auto opacity-60">{optionLetter === 'A' ? 'A/1' : optionLetter === 'B' ? 'B/2' : optionLetter === 'C' ? 'C/3' : 'D/4'}</Badge>}
        ...
        <Card className="mb-4 shadow-lg rounded-xl overflow-hidden bg-white dark:bg-gray-800 animate-fade-in"><CardHeader className="p-4 border-b dark:border-gray-700"><CardTitle className="text-lg font-bold text-gray-800 dark:text-gray-200 flex items-center gap-2"><Target className="h-5 w-5 text-blue-600" /> Bản đồ câu hỏi</CardTitle></CardHeader><CardContent className="p-4"><div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-3">{practiceData.questions.map((_, index) => { const result = questionResults[index + 1]; const isActive = currentQuestion === index; let dotClass = "relative w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center text-white font-bold text-sm transition-all duration-300 ease-in-out cursor-pointer shadow-md"; if (isActive) dotClass += " bg-blue-600 ring-4 ring-blue-300 scale-110 z-20"; else if (result?.answered) dotClass += result.correct ? " bg-green-500 hover:bg-green-600" : " bg-red-500 hover:bg-red-600"; else dotClass += " bg-gray-400 hover:bg-gray-500 dark:bg-gray-600 dark:hover:bg-gray-500"; return <Button key={index} variant="ghost" className={`${dotClass}`} onClick={(e) => goToQuestion(index, e)}>{index + 1}</Button>; })}</div></CardContent></Card>

        {/* --- Summary Dialog --- */}
        <AlertDialog open={showSummaryDialog} onOpenChange={setShowSummaryDialog}>{/* ... (same as original) ... */}</AlertDialog>

        {/* --- NEW: Shortcuts Help Dialog --- */}
        <AlertDialog open={showShortcutsHelp} onOpenChange={setShowShortcutsHelp}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>Sử dụng các phím sau để điều khiển nhanh hơn:</AlertDialogDescription>
            </AlertDialogHeader>
            <div className="text-sm space-y-2 font-sans">
              {[
                { label: "Chọn đáp án A, B, C, D", keys: ["A/1", "B/2", "C/3", "D/4"] },
                { label: "Câu tiếp theo", keys: ["→", "N", "Space"] },
                { label: "Câu trước đó", keys: ["←", "P"] },
                { label: "Làm lại câu hỏi", keys: ["R"] },
                { label: "Hiện/Ẩn gợi ý", keys: ["H"] },
                { label: "Về trang chủ", keys: ["Esc"] },
                { label: "Mở bảng này", keys: ["?"] },
              ].map(shortcut => (
                <div key={shortcut.label} className="flex justify-between items-center p-2 rounded-md bg-gray-100 dark:bg-gray-800">
                  <span className="text-gray-700 dark:text-gray-300">{shortcut.label}</span>
                  <div className="flex gap-1.5">{shortcut.keys.map(k => <kbd key={k} className="pointer-events-none inline-flex h-6 select-none items-center justify-center rounded border bg-muted px-2 font-mono text-sm font-medium text-muted-foreground">{k}</kbd>)}</div>
                </div>
              ))}
            </div>
            <AlertDialogFooter>
              <AlertDialogAction onClick={() => setShowShortcutsHelp(false)}>Đã hiểu</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
