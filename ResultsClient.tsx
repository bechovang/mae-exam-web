  ...
  useEffect(() => {
    const loadResults = async () => {
      ...
      try {
        ...
        const response = await fetch(`/data/de${examId}.json`)
        const examData = await response.json()
        
        const parsedData = JSON.parse(resultsData)
        // Use questions from the JSON file
        parsedData.questions = examData.questions.map((q: ExamQuestion) => ({
          id: q.id,
          image: `/images/de${examId}/${q.id}.jpg`,
          answer: q.answer
        }))
        ...
        // Fallback to default behavior if JSON file cannot be loaded
        const parsedData = JSON.parse(resultsData)
        if (!parsedData.questions) {
          parsedData.questions = Array.from({ length: parsedData.totalQuestions }, (_, i) => ({
            id: i + 1,
            image: `/images/de${examId}/${i + 1}.jpg`,
            answer: "A" // Default answer if JSON fails to load
          }))
        }
        setResults(parsedData)
      }
